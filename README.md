...
- **Shelly.** A web wizard asks three questions and generates a device script with the answers
  built in, which then runs the plan on the plug itself.
- **Home Assistant.** A custom integration in its own repository,
  [spotprice-ha](https://github.com/hurtamat/spotprice-ha), installed through HACS and set up
  entirely in the UI.
...
| --- | --- | --- |
| `spotPriceCalc/` | C#, ASP.NET Core, EF Core, Npgsql | REST API, daily ENTSO-E ingestion, scheduling, rate limiting |
| `calc-service/` | Python, FastAPI, pandas, NumPy | Cheap / expensive cut-offs from de-trended price residuals |
