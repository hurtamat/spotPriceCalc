...
device-agnostic request and neither optimises anything itself; they differ only in what comes back.
Home Assistant gets the run blocks plus the price curve, a Shelly gets a flatter shape sized for a
device with roughly 8 KB of script heap.

| Client | Endpoint | Where it lives |
| --- | --- | --- |
| Shelly (Gen2+) | `POST /api/shelly/schedule`, `GET /api/shelly/schedule/status` | [`scripts/shelly/`](./scripts/shelly/README.md), generated by the wizard at `/shelly` |
